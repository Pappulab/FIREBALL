...
def optimizer(mode,
    ...
    fit = minimize(partial_binodal.residual_finder, free_parameter_list, (fixed_parameter_list, free_parameter_checklist, mode, regime, dilute_array, dense_array, temperature_offset, silent), method = fitting_method, options={'maxfev': maxfev, 'xatol': xatol, 'fatol': fatol})
    ...
    if fitting_method == 'Nelder-Mead':
        ...
        final_centroid_func = partial_binodal.residual_finder(final_centroid, fixed_parameter_list, free_parameter_checklist, mode, regime, dilute_array, dense_array, temperature_offset, silent=True)
        ...
        while threshold_check:
            ...
            for index, param_set in enumerate(new_params):
                new_funcs[index] = partial_binodal.residual_finder(param_set, fixed_parameter_list, free_parameter_checklist, mode, regime, dilute_array, dense_array, temperature_offset, silent=True)
        ...
        for i in range(num_vertices):
            for j in range(i + 1):
                ...
                new_func_matrix[i, j] = partial_binodal.residual_finder(new_params_array[i][j], fixed_parameter_list, free_parameter_checklist, mode, regime, dilute_array, dense_array, temperature_offset, silent=True)
        ...
        num_points = dilute_array.shape[0] + dense_array.shape[0]
